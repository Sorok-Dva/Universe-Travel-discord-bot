 ...
 *  |__   _|/ __/                          Updated: 2021/06/21 7:34 PM
 *     |_| |_____|U*Travel
 ******************************************************************************/
      ...
      switch (module) {
        case 'system':
          si.osInfo((data: Record<string, string>) => {
            ...
            message.reply({embed}).catch(err => errors.raiseReply(err, message));
...
const advancedCPUDebug = (message: Message) => {
  let embed;
...
const batteryDebug = (message: Message) => {
  let embed;
